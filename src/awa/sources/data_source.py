...
class AIHRCReports(DataSource):
    ...
    def find_links(self):
        ...
        other_parts = [
            x
            for x in links
            if segment_link_pattern.search(x.get("href")) is not None
            and segment_name_pattern.search(x.text) is not None
        ]
        urls = [self.url] + [
            urljoin(self.url, link.get("href")) for link in other_parts
        ]
